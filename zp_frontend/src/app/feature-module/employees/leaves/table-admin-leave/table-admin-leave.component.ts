...
@Component({
  selector: 'app-table-admin-leave',
  templateUrl: './table-admin-leave.component.html',
  styleUrls: ['./table-admin-leave.component.scss']
})
export class TableAdminLeaveComponent implements OnInit {
  selected1 = 'option1';
  selected2 = 'option2';
  selected3 = 'option3';
  ...
  constructor(private leaveService:LeaveService,private formBuilder: FormBuilder,
              private model :ModalService,private toaste:ToasterService
            ) {}
  ...
  ngOnChange(){
    if(this.leaveToEdit){
      ...
      this.remainLeaves=this.leaveToEdit.user.dayoffamount
    }
  }
  ...
  editLeaveReaquest() {
    ...
    this.leaveService.editLeaveRequest(formData, this.leaveToEdit.id).subscribe(
      (res) => {
        ...
        this.model.closeModel('edit_leave_admin');
      },
      (error) => {
        this.model.closeModel('edit_leave_admin');
      })
  ...
  deleteLeaveReqest() {
    this.leaveService.deleteLeaveRequest(this.leaveToEdit.id).subscribe((res) => {
      ...
      this.model.closeModel('delete_admin_leave');
    },
      (error) => {
        this.model.closeModel('delete_admin_leave');
      })
